...
#   2) Batched REST pricing + preliminary filters
...
if st.sidebar.button("Run Bargain Hunter"):
    ...
    for b in range(total_batches):
        ...
        for v, sku, fmt in batch:
            ...
            entry = entries[0]
            raw_ask, raw_mkt, raw_last = (entry.get("least_listing_price"),
                                          entry.get("market_price"),
                                          entry.get("last_bbx_transaction"))
            ...
            if any(fails):
                reasons = []
                if fails[0]: reasons.append(f"mkt {pct_market}% < {min_pct_market}%")
                if fails[1]: reasons.append(f"price £{ask_price} > £{max_price}")
                if fails[2]: reasons.append(f"last {pct_last}% < {min_pct_last}%")
                debug2.append({**rec, "pct_market":pct_market, "pct_last":pct_last,
                               "reason":"; ".join(reasons), "passed": False})
                continue
            ...
            debug2.append({**rec, "pct_market": pct_market, "pct_last": pct_last, "reason":"passed", "passed":True})
    ...
    for idx, v in enumerate(rest_candidates, start=1):
        ...
        try:
            ...
            if case_fmt != "Any":
                ok = any(case_fmt in variant.get("product",{}).get("item_name","")
            ...
            if next_lowest is not None:
                pct_next = round((next_lowest-lowest)/next_lowest*100,1)
