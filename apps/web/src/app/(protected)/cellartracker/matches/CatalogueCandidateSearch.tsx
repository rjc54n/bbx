...
import {
  ...
  searchCellarTrackerCatalogue,
  type CellarTrackerCatalogueSearchState,
} from "./actions";

const initialState: CellarTrackerCatalogueSearchState = { results: [] };

export function CellarTrackerCatalogueSearch({
  matchGroupKey,
  sourceWine,
  defaultQuery,
  sourceVintage,
  ...
  const [state, action, pending] = useActionState(searchCellarTrackerCatalogue, initialState);
  return <details className="mt-3">
    ...
    <form action={action} className="mt-2 flex flex-wrap gap-2">
      <input type="search" name="query" defaultValue={defaultQuery || sourceWine} className="min-w-64 flex-1 rounded border border-border px-2 py-1.5 text-sm" />
    ...
    {state.results.length > 0 && <ul className="mt-2 divide-y divide-border rounded border border-border">
      {state.results.map((candidate) => <li key={candidate.parent_sku} className="flex items-start justify-between gap-3 p-2 text-xs">
        ...
        <form action={confirmCellarTrackerCandidate.bind(null, matchGroupKey, candidate.parent_sku, returnPath)}><button className="rounded border border-accent px-2 py-1 text-accent">Confirm</button></form>
      </li>)}
    </ul>}
  </details>;
}
