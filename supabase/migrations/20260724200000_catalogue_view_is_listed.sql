-- Phase 4 Step 7: expose skus.is_listed as a passthrough in catalogue_view.
-- is_listed is reconciled against Algolia discovery every run
-- (core/sweep.py _reconcile_listing_state), independent of REST tiering --
-- do not re-derive it here as `ask IS NOT NULL`, which is exactly the bug
-- the 2026-07-24 external review follow-up fixed: a wine can lose its
-- listing without being REST-repriced the same day under wave pricing.

-- CREATE OR REPLACE VIEW requires existing output columns to retain their
-- names, types and ordinal positions, so is_listed is appended after
-- last_rest_checked_at (the current last column) via a fresh join back to
-- skus in the outermost SELECT, rather than threaded through the base
-- subquery's `.*` wildcards -- inserting it there would shift every column
-- computed after it (price_vs_next_pct onward). The join is 1:1: skus'
-- primary key is (parent_sku, format_code), the same pair catalogue_view
-- already exposes and that fetchCatalogue.ts pages on.
...
JOIN skus sk ON sk.parent_sku = priced.parent_sku AND sk.format_code = priced.format_code;
